...
namespace CommonServices.Messaging
{
    public abstract class MessageBusBase<TOptions> : MaintenanceBase, IMessageBus where TOptions : MessageBusOptionsBase
    {
        ...
        //public MessageBusBase(ILoggerFactory logFactory) : base(logFactory)
        //{
        //    MessageBusId = "Default_" + Guid.NewGuid().ToString("N").Substring(10);
        ...
        protected async Task SendMessageToSubscribersAsync(MessageBusData message)
        {
            ...
            try
            {
                body = JsonConvert.DeserializeObject(message.Data, messageType);
            }
            catch (Exception ex)
            {
